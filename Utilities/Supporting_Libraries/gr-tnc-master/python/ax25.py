...
def printpacket(pkt):
    ...
    if j:
        string+="\n"
    return string,payload_str,pid,ctrl,src,dst
...
def sendpacket(packet,preamble,postamble):
    global out,flag,fcs,stuff,fcslo,fcshi
    out=[]
    fcslo=0xff
    fcshi=0xff
    ...
    fcs=False
    #head flags
    for i in range(preamble): sendbyte(0x7e)
    flag=False
#    print len(packet)
    ...
    flag=True
    #tail flags
    for i in range(postamble): sendbyte(0x7e)
    return out
